        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    ...
                    <div class="flex items-center">
                        @if($work->cover_image)
                            <img src="{{ asset('storage/' . $work->cover_image) }}" alt="{{ $work->title }}" class="w-24 h-24 object-cover rounded mr-4">
                        @else
                        ...
                        @endif
                        <div>
                            <h4 class="text-xl font-bold">{{ $work->title }}</h4>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-semibold mb-4">{{ __('評価を投稿する') }}</h3>
                    
                    <form method="POST" action="{{ route('work-evaluations.store') }}">
                        @csrf
                        ...
                        <div class="mt-6">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-2">
                                <div class="flex items-center">
                                    <input id="helped" type="checkbox" name="helped" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('helped') ? 'checked' : '' }}>
                                    <label for="helped" class="ml-2 text-sm text-gray-600">{{ __('助かりました〜') }}</label>
                                </div>
                                
                                <div class="flex items-center">
                                    <input id="grateful" type="checkbox" name="grateful" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('grateful') ? 'checked' : '' }}>
                                    <label for="grateful" class="ml-2 text-sm text-gray-600">{{ __('圧倒的感謝') }}</label>
                                </div>
                                
                                <div class="flex items-center">
                                    <input id="amazing" type="checkbox" name="amazing" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('amazing') ? 'checked' : '' }}>
                                    <label for="amazing" class="ml-2 text-sm text-gray-600">{{ __('すごすぎ！') }}</label>
                                </div>
                                
                                <div class="flex items-center">
                                    <input id="expectation" type="checkbox" name="expectation" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('expectation') ? 'checked' : '' }}>
                                    <label for="expectation" class="ml-2 text-sm text-gray-600">{{ __('期待してます') }}</label>
                                </div>
                                
                                <div class="flex items-center">
                                    <input id="kind" type="checkbox" name="kind" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('kind') ? 'checked' : '' }}>
                                    <label for="kind" class="ml-2 text-sm text-gray-600">{{ __('優しい〜') }}</label>
                                </div>
                                
                                <div class="flex items-center">
                                    <input id="genius" type="checkbox" name="genius" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('genius') ? 'checked' : '' }}>
                                    <label for="genius" class="ml-2 text-sm text-gray-600">{{ __('天才！？') }}</label>
                                </div>
                                
                                <div class="flex items-center">
                                    <input id="encouragement" type="checkbox" name="encouragement" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('encouragement') ? 'checked' : '' }}>
                                    <label for="encouragement" class="ml-2 text-sm text-gray-600">{{ __('頑張れ！') }}</label>
                                </div>
                                
                                <div class="flex items-center">
                                    <input id="perfect_score" type="checkbox" name="perfect_score" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('perfect_score') ? 'checked' : '' }}>
                                    <label for="perfect_score" class="ml-2 text-sm text-gray-600">{{ __('100点満点！') }}</label>
                                </div>
                                
                                <div class="flex items-center">
                                    <input id="excellent" type="checkbox" name="excellent" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('excellent') ? 'checked' : '' }}>
                                    <label for="excellent" class="ml-2 text-sm text-gray-600">{{ __('最高です') }}</label>
                                </div>
                                
                                <div class="flex items-center">
                                    <input id="good_work" type="checkbox" name="good_work" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('good_work') ? 'checked' : '' }}>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea id="comment" name="comment" rows="4" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">{{ old('comment') }}</textarea>
